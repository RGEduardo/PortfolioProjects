...
plt.savefig('images/linear_trendline.png')
...
# Save the graph as an image
plt.savefig('images/seasonal_avg_temperature.png')

# Calculate and print the season with the highest average temperature
# Calculate and print the season with the highest and lowest average temperature
max_season = seasonal_avg_temp.mean(axis=0).idxmax()
max_temp = seasonal_avg_temp.mean(axis=0).max()
min_season = seasonal_avg_temp.mean(axis=0).idxmin()
min_temp = seasonal_avg_temp.mean(axis=0).min()
...
print(f"The season with the lowest average temperature in Mexico City is {min_season} with an average temperature of {min_temp:.2f} °C.")
...
# Save the fourth graph as an image
plt.savefig('images/annual_avg_temperature.png')
